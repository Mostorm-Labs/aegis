...
class CpI05DispatchTests(unittest.TestCase):
    def setUp(self):
        ...
        self.dispatch = DispatchService(self.store, self.surface)
    ...
    def test_duplicate_transport_keeps_one_semantic_execution_identity(self):
        first = self.dispatch.dispatch(
            self.outbox_id, dispatch_authorized=True, attempted_at="2026-09-01T02:10:00Z"
        )
        second = self.dispatch.dispatch(
            self.outbox_id, dispatch_authorized=True, attempted_at="2026-09-01T02:10:01Z"
        )
    ...
    def test_unauthorized_dispatch_never_calls_provider(self):
        ...
        with self.assertRaises(DispatchRejected) as caught:
            self.dispatch.dispatch(
                self.outbox_id, dispatch_authorized=False, attempted_at="2026-09-01T02:10:00Z"
            )
    ...
    def test_missing_outbox_cannot_dispatch(self):
        with self.assertRaises(DispatchRejected) as caught:
            self.dispatch.dispatch(
                "out_missing", dispatch_authorized=True, attempted_at="2026-09-01T02:10:00Z"
            )
